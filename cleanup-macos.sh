...
echo -e "\n${CYAN}========================================${NC}"
echo -e "${CYAN}BypassAIGC 清理工具 (macOS)${NC}"
echo -e "${CYAN}========================================${NC}\n"

echo -e "${YELLOW}选择清理选项:${NC}\n"
echo "1) 清理临时文件和日志"
echo "2) 停止所有服务（包括 tmux 会话）"
echo "3) 删除数据库（保留配置）"
echo "4) 完全重置（删除虚拟环境和依赖）"
echo "5) 清理编译文件和缓存"
echo "6) 卸载 launchd 服务"
echo "0) 退出"
echo ""
...
case $choice in
    1)
        ...
        rm -rf "$SCRIPT_DIR/logs" 2>/dev/null && echo "✓ 删除日志目录"
        ...
        # macOS 特定的缓存
        rm -f "$SCRIPT_DIR/.DS_Store" 2>/dev/null && echo "✓ 删除 .DS_Store"
    ...
    4)
        ...
        echo -e "${RED}您需要重新运行 ./setup-macos.sh${NC}"
        ...
        if [ "$confirm" = "yes" ]; then
            ...
            echo -e "\n${GREEN}✓ 环境重置完成${NC}"
            echo -e "${CYAN}请运行 ./setup-macos.sh 重新配置环境${NC}\n"
        else
            echo -e "${YELLOW}已取消${NC}\n"
        fi
        ;;
    ...
    5)
        ...
        # macOS 特定文件
        find "$SCRIPT_DIR" -name ".DS_Store" -delete 2>/dev/null && echo "✓ 清理 .DS_Store 文件"
        
        echo -e "\n${GREEN}✓ 缓存清理完成${NC}\n"
        ;;
    
    6)
        echo -e "\n${YELLOW}卸载 launchd 服务...${NC}"
        
        # 检查是否存在 launchd 配置
        BACKEND_PLIST="$HOME/Library/LaunchAgents/com.bypassaigc.backend.plist"
        FRONTEND_PLIST="$HOME/Library/LaunchAgents/com.bypassaigc.frontend.plist"
        
        if [ -f "$BACKEND_PLIST" ]; then
            launchctl unload "$BACKEND_PLIST" 2>/dev/null
            rm "$BACKEND_PLIST"
            echo -e "${GREEN}✓ 已卸载后端服务${NC}"
        else
            echo -e "${YELLOW}后端服务未安装${NC}"
        fi
        
        if [ -f "$FRONTEND_PLIST" ]; then
            launchctl unload "$FRONTEND_PLIST" 2>/dev/null
            rm "$FRONTEND_PLIST"
            echo -e "${GREEN}✓ 已卸载前端服务${NC}"
        else
            echo -e "${YELLOW}前端服务未安装${NC}"
        fi
        
        echo -e "\n${GREEN}✓ launchd 服务清理完成${NC}\n"
        ;;
