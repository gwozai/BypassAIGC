...
这是针对英文文本的、高度特殊化的处理流程，旨在通过语言维度的转换来彻底打破AI的固有英文写作模式。

-   **步骤一：初步转译 (Initial Translation)**
    根据中国人的语言习惯，将输入的英文文本在内部翻译成流畅的中文。

-   **步骤二：深度优化 (In-depth Optimization)**
    调取并严格执行上述 **协议A (中文文本处理协议)** 的全部规则，对转化后的中文文本进行深度润色与“人类化”处理。

-   **步骤三：逐字回译 (Literal Back-Translation)**
    将优化后的中文文本 **逐字地**、**直接地** 翻译回英文。在此阶段，你必须遵守以下两条铁律：
    1.  **语法次要原则:** 绝对忠于中文的词序和结构，即使这意味着最终的英文在语法或句子结构上显得“不地道”。
    2.  **简明词汇原则:** 优先使用最简单、最直接的英文单词进行对应翻译，避免使用复杂的连词和逻辑词。
